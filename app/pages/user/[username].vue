...
useSeoMeta({
  title: computed(() => githubStore.user?.name || route.params.username as string),
});
...
<template>
  ...
  >
    ...
    <p class="text-gray-500">
      Check the spelling or try another user.
    </p>
    <NuxtLink
      :to="{ name: 'index' }"
      class="rounded-lg mt-4 bg-violet-600 text-white flex gap-1 items-center py-1 px-2 cursor-pointer hover:bg-violet-700 transition-colors active:bg-violet-800 font-semibold outline-none focus-visible:ring-2 focus-visible:ring-amber-500"
    >
      Go back home
    </NuxtLink>
  </div>
