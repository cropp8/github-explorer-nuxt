...
<template>
  <template v-if="githubStore.user && githubStore.repos.length > 0">
    <h2 class="col-span-1 md:col-span-8 lg:col-span-9 text-2xl font-semibold py-2">
      ...
      <span class="inline-block px-1 rounded-sm bg-gray-200 dark:bg-gray-900 dark:text-gray-400">@{{ githubStore.user.login }}</span>
    </h2>
    ...
    <div
      v-if="totalPages > 1"
      class="col-span-1 md:col-span-8 lg:col-span-9 mt-6"
    >
      <UiPagination
  ...
  >
    <div class="text-xl text-center dark:text-gray-400">
